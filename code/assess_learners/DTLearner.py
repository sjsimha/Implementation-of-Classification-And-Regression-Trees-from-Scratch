...
class DTLearner(object):
    ...
    def query(self, points):
        ...
        if self.verbose:
            np.savetxt('Data/input.txt', points, delimiter='\t', fmt='%f')
            np.savetxt('Data/dtree.txt', self.dtree, delimiter='\t', fmt='%f')
            np.savetxt('Data/output.txt', pred_y, delimiter='\t', fmt='%f')
    ...
    def build_dtree(self, in_data):
        if in_data.shape[0] <= self.leaf_size:
            ...
            return self.generate_base_case_return(self.get_mean_y(in_data))
        else:
            y = in_data[:, -1]
            if np.all(y == y[0]):
                # all labels are same
